...
Route::get('/dashboard', function () {
    ...
    // 1. Hitung Statistik untuk 3 Card Ringkasan
    $activeReservations  = Reservation::where('user_id', $userId)->where('status', 'disetujui')->count();
    $pendingReservations = Reservation::where('user_id', $userId)->where('status', 'menunggu')->count();
    $reportsCount        = Report::where('user_id', $userId)->count();

    // 2. Ambil Data Riwayat Reservasi (Langsung dengan relasi facility)
    $reservations = Reservation::where('user_id', $userId)
        ->with('details.facility')
        ->latest('start_time')
        ->get();

    // 3. Ambil Data Riwayat Laporan Kerusakan
...
// Redirect alias /pengguna ke /dashboard agar tidak ada duplikasi kode
...
// --- RUTE PETUGAS (DIPROTEKSI MIDDLEWARE) ---
Route::middleware(['auth', 'account.status', 'role:petugas,admin'])->group(function () {
...
Route::get('/api/facilities/{facility}/slots', function (Request $request, \App\Models\Facility $facility) {
    $date = $request->query('date', \Carbon\Carbon::today('Asia/Jakarta')->toDateString());
    
    $slots = app(FacilityController::class)->getSlotsForDate($facility, $date);

    return response()->json([
        'date' => $date,
        'formatted_date' => \Carbon\Carbon::parse($date)->translatedFormat('l, d F Y'),
        'slots' => $slots,
    ]);
